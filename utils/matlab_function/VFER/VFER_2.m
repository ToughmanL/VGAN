...
for categ_index = 3 : length(category)
    ...
    for person_index = 3 : length(person)
        % if person(person_index).name == "S_00005_M" || person(person_index).name == "S_00032_M"
        ...
        data = [];
        data = [name, VFER_MEAN_ZHUAN, VERF_SEO_SNR_MEAN_ZHUAN, VERF_TKEO_SNR_MEAN_ZHUAN];
        data(isnan(VFER_MEAN_ZHUAN), :) = [];
        [m, n] = size(data);
        data_cell = [];
        data_cell = mat2cell(data, ones(m,1), ones(n,1));    % 将data切割成m*n的cell矩阵
        title = {'Filename', 'VFER_MEAN', 'VFER_SEO', 'VERF_TKEO_SNR_MEAN_ZHUAN'};
        result = [];
